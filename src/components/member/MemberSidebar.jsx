...
const menuItems = [
  { icon: MdDashboard,          label: "Dashboard",      path: "/member"          },
  { icon: MdExplore,            label: "Jelajah Paket",  path: "/member/explore"  },
  { icon: MdConfirmationNumber, label: "Booking Saya",   path: "/member/bookings" },
  { icon: MdPayment,            label: "Pembayaran",     path: "/member/payments" },
  { icon: MdFavoriteBorder,     label: "Wishlist",       path: "/member/wishlist" },
  { icon: MdPerson,             label: "Profil Saya",    path: "/member/profile"  },
];
    ...
    <aside className="w-[220px] flex-shrink-0 hidden md:block">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sticky top-20">
        ...
        {menuItems.map(({ icon: Icon, label, path }) => (
          <NavLink key={label} to={path} end className={linkClass}>
        ...
        <div className="mt-4 mx-1 p-3 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-100">
          <p className="text-[11px] text-gray-500 mb-1">Loyalty Tier</p>
          <p className="text-[15px] font-extrabold text-biru">🥈 Silver</p>
        </div>
      </div>
    </aside>
  );
